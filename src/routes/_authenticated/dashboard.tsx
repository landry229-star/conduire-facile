...
type TheoryProgress = Database["public"]["Tables"]["theorie_progress"]["Row"];
...
function Dashboard() {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const firstError = [p, r, l, q, e, h, pay].find(
                    (response) => response.error,
                )?.error;
                if (firstError) throw firstError;

                setProfile(p.data);
                setRoles(r.data?.map((x: UserRole) => x.role) ?? []);
    ...
    const recentAttempts = [...quizAttempts, ...examAttempts] as Array<QuizAttempt | ExamAttempt>;
            ...
            <main className="max-w-6xl mx-auto space-y-6">
                <div>
                    ...
                    <p className="text-slate-600 text-sm">
                        Voici votre tableau de bord de formation.
                    </p>
                    <div className="flex gap-2 mt-2">
                        ...
                        {profile?.category && (
                            <Badge variant="outline">Catégorie {profile.category}</Badge>
                        )}
                    </div>
                </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs mt-1 text-slate-500">
                                {lessonsDone} / {TOTAL_LESSONS} leçons
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-xs text-slate-500">
                                {quizAttempts.length
                                    ? `Dernier : ${quizAttempts[0].score}/${quizAttempts[0].total}`
                                    : "Aucune tentative"}
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <CardTitle className="text-3xl">
                                {examAttempts.filter((e) => e.passed).length}
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-xs text-slate-500">
                                {examAttempts.length} tentatives
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="text-sm text-slate-600">
                            Suivez la progression dans le menu latéral : <strong>Cours</strong> pour
                            la théorie et les panneaux, puis <strong>Évaluations</strong> pour les
                            quiz et l’examen de la catégorie {profile?.category || "choisie"}.
                        </p>
                    </CardContent>
                </Card>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-2 text-sm">
                            {recentAttempts
                                ...
                                .slice(0, 6)
                                .map((a) => {
                                    const isQuiz = "quiz_type" in a;
                                    const label = isQuiz
                                        ? `Quiz ${a.quiz_type}`
                                        : `Examen ${a.category || "blanc"}`;
                                    const score = a.score;
                                    const total = a.total;
                                    const failed = !isQuiz && a.passed === false;
                            ...
                            {quizAttempts.length + examAttempts.length === 0 && (
                                <p className="text-slate-500">
                                    Aucun historique. Commencez par un quiz !
                                </p>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                ...
                                {grandTotal.toLocaleString("fr-FR")} FCFA payés
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-2 text-sm">
                            {payments.length === 0 && (
                                <p className="text-slate-500">
                                    Aucune tranche enregistrée. Voyez avec l'administration.
                                </p>
                            )}
                            {payments.map((p) => (
                                <div
                                ...
                                >
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-500">
                                            {p.paid
                                                ? `Payé le ${new Date(p.paid_at).toLocaleDateString("fr-FR")}${p.method ? " · " + p.method : ""}`
                                                : "En attente"}
                                        </div>
                                    </div>
