...
function QuestionsPage() {
    ...
    const load = async () => {
        ...
        const [c, s, q, l] = await Promise.all([
            ...
            supabase.from("exam_question_categories").select("*"),
        ]);
        setCategories(c.data ?? []);
        setSkills(s.data ?? []);
        setQuestions(q.data ?? []);
        const map: Record<string, string[]> = {};
        (l.data ?? []).forEach((r: { question_id: string; category_id: string }) => {
                        ...
                        initial={
                            editing
                                ? {
                                      ...
                                      choices: (editing.choices?.length
                                          ? editing.choices
                                          : ["", "", "", ""]) as string[],
...
function QuestionForm({
    ...
    const save = async () => {
        ...
        if (questionId) {
            await supabase.from("exam_question_categories").delete().eq("question_id", questionId);
            if (draft.categoryIds.length > 0) {
                const rows = draft.categoryIds.map((cid) => ({
                    question_id: questionId,
                    category_id: cid,
                }));
                const { error } = await supabase.from("exam_question_categories").insert(rows);
                if (error) {
                    setSaving(false);
                    return toast.error(error.message);
                }
            ...
            <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <CardDescription>
                            Rattachez-la à une compétence et une ou plusieurs catégories.
                        </CardDescription>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            onValueChange={(v) =>
                                setDraft({ ...draft, skill_id: v === "none" ? null : v })
                            }
                        >
                            <SelectTrigger>
