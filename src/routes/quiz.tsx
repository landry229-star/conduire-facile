...
import { ensureCourseCompleted } from "@/lib/access-control";

export const Route = createFileRoute("/quiz")({
    beforeLoad: async ({ location }) => {
        await ensureCourseCompleted(location.pathname);
    },
    head: () => ({
        meta: [
        ...
        questions: [
            {
                ...
                answer: 1,
                explain:
                    "La catégorie AM est accessible dès 14 ans pour les cyclomoteurs ≤ 50 cm³.",
            },
            {
    ...
    },
    {
        ...
        questions: [
            ...
            },
            {
                ...
                answer: 1,
                explain:
                    "0,5 g/L de sang est la limite courante ; tolérance zéro pour les novices.",
            },
            {
            ...
            },
            {
                ...
                answer: 2,
                explain:
                    "Un contrôle technique périodique est obligatoire (voir réglementation locale).",
            },
        ],
    },
    {
        ...
        questions: [
            {
                ...
                answer: 0,
                explain:
                    "Vérification obligatoire des issues, extincteur et signalétique de sécurité.",
            },
            {
            ...
            },
            {
                ...
                answer: 1,
                explain:
                    "Sécuriser les passagers hors du véhicule, derrière la glissière + triangle.",
            },
        ],
    },
    {
...
function QuizPage() {
    ...
    const active = useMemo(() => quizzes.find((q) => q.slug === activeSlug) ?? null, [activeSlug]);
    const availableQuizzes = useMemo(() => {
        if (!category) return [];
        const slug = ["AM", "A1", "A2", "A"].includes(category)
            ? "moto"
            : category === "C"
              ? "poids-lourd"
              : category === "D"
                ...
                : category === "T"
                  ? "agricole"
                  : "voiture";
        return quizzes.filter((quiz) => quiz.slug === slug);
    }, [category]);
    ...
    const score = useMemo(() => {
        if (!active) return 0;
        return active.questions.reduce((acc, q, i) => acc + (answers[i] === q.answer ? 1 : 0), 0);
    }, [active, answers]);
        ...
        <div className="min-h-screen bg-ivory text-charcoal">
            ...
            <nav className="sticky top-0 z-40 flex items-center justify-between border-b border-charcoal/5 bg-ivory/90 px-5 py-4 backdrop-blur-md">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        <Signpost className="size-3" /> Quiz panneaux
                    </Link>
                    <Link
                        to="/dashboard"
            ...
            {!active ? (
                <section className="px-5 py-12">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        {!category && (
                            <p className="rounded-xl bg-benin-yellow/10 p-5 text-sm text-charcoal/70">
                                Choisissez d’abord votre catégorie de permis dans votre profil pour
                                recevoir le cours et le quiz adaptés.
                            </p>
                        )}
                    </div>
                </section>
                ...
                <section className="px-5 py-10">
                                ...
                                >
                                    ...
                                    {submitted && (
                                        <p className="mt-3 rounded-md bg-zinc-50 p-3 text-xs leading-relaxed text-charcoal/70">
                                            <span className="font-semibold text-benin-green">
                                                Réponse :
                                            </span>{" "}
                                            {q.explain}
                                        </p>
                    ...
                    {!submitted ? (
                        <button
                            type="button"
                            onClick={async () => {
                                const orderedAnswers = active.questions.map(
                                    (_, index) => answers[index] ?? -1,
                                );
                                const { data, error } = await supabase.rpc("submit_quiz_attempt", {
                                    p_quiz_type: active.slug,
                        ...
                        <div className="mt-6 rounded-xl bg-white p-6 ring-1 ring-black/5">
                            ...
                            <p className="mt-3 text-sm leading-relaxed text-charcoal/70">
                                {(serverScore ?? score) / active.questions.length >= 0.8
                                    ? "Excellent ! Vous avez les bases pour démarrer sereinement la formation pratique."
                                    : (serverScore ?? score) >=
                                        Math.ceil(active.questions.length * 0.5)
                            ...
                            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                                ...
                                <Link
                                    to="/quiz-panneaux"
                                    className="inline-flex items-center justify-center gap-2 rounded-md bg-benin-green px-4 py-3 text-sm font-medium text-white"
                                >
                                    Quiz panneaux <ArrowRight className="size-4" />
                                </Link>
                                <Link
                                    to="/examen"
                                    className="inline-flex items-center justify-center gap-2 rounded-md bg-charcoal px-4 py-3 text-sm font-medium text-white"
                                >
                                    Examen blanc <ArrowRight className="size-4" />
                                </Link>
                                <button
                                    type="button"
                                    onClick={resetQuiz}
                    ...
                    {submissionError && (
                        <p className="mt-3 rounded-md bg-benin-red/10 p-3 text-sm text-benin-red">
                            {submissionError}
                        </p>
                    )}
                </section>
            ...
            <footer className="border-t border-charcoal/5 bg-zinc-50 px-5 py-10">
                <p className="text-[11px] leading-relaxed text-charcoal/50">
                    © {new Date().getFullYear()} L'Excellence Auto-École. Quiz à but pédagogique,
                    non officiel.
                </p>
            </footer>
        </div>
